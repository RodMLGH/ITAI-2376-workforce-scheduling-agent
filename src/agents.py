...
def create_tasks(
    ...
    schedule_task = Task(
        description=(
            "Use the Scheduling Optimization Tool to generate the workforce schedule. "
            "Then use the Schedule Validation Tool to validate the schedule. "
            "Finally, use the Schedule Explanation Tool to create the manager-friendly "
            "explanation. Confirm that schedule, validation, and explanation files "
            "were created."
        ),
        expected_output=(
            "A final summary confirming that the OR-Tools schedule, validation "
            "summary, and LLM explanation were generated successfully."
        ),
        agent=scheduling_explanation_agent,
        context=[forecast_task],
    )

    return forecast_task, schedule_task
...
def main() -> None:
    ...
    forecast_task, schedule_task = create_tasks(demand_agent, scheduling_agent)

    crew = Crew(
        agents=[demand_agent, scheduling_agent],
        tasks=[forecast_task, schedule_task],
